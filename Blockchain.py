...
class Blockchain:
    ...
    def __process_transactions(self, transactions):
        ...
        for i in range(len(transactions)):
            sender = transactions[i]['message']['sender']
            receiver = transactions[i]['message']['receiver']
            transfer_amount = transactions[i]['message']['value']
            if transfer_amount > self._accounts[sender].balance:
                return False
            else:
                for account in list(self._accounts.values()):
                    if account.id == sender:
                        account.decrease_balance(transfer_amount)

                    if account.id == receiver:
                        account.increase_balance(transfer_amount)
        return True
    ...
    def __validate_chain_hash_integrity(self):
        ...
        # Return False otherwise
        for index in range(self._chain.__len__()):
            if index > 0:
                if self._chain[index].previous_block_hash != self._chain[index-1].block_hash:
                    return False
        return True
    ...
    def __validate_block_hash_target(self):
        ...
        # Return False otherwisehash_target = {str} '000fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'

        for index in range(self._chain.__len__()):
            if index > 0:
                if int(self._chain[index].block_hash, 16) > int(self._hash_target, 16):
                    return False
        return True
    ...
    def __validate_complete_account_balances(self):
        ...
        for index in range(self._chain.__len__()):
            if index > 0:
                for transaction in self._chain[index]._transactions:
                    sender = transaction['message']['sender']
                    if transaction['message']['value'] > self._accounts[sender].balance:
                        return False
        return True
